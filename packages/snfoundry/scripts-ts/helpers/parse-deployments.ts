...
const TARGET_DIR = path.join(__dirname, "../../../nextjs/contracts-strk");
const INDEXER_TARGET_DIR = path.join(__dirname, "../../../indexer");
const INDEXER_ABI_DIR = path.join(__dirname, "../../../indexer/abis");
...
const generateTsAbis = async () => {
  ...
  fs.writeFileSync(
    path.join(INDEXER_ABI_DIR, "deployedContracts.strk.ts"),
    formattedContent
  );

  // override env file in indexer
  if (!fs.existsSync(INDEXER_TARGET_DIR)) {
    throw new Error("Indexer target directory does not exist");
  }
  const envPath = path.join(INDEXER_TARGET_DIR, ".env");
  const envContent = fs.readFileSync(envPath, "utf8");

  // Replace HTLC_STARK value while preserving other env vars
  const updatedEnvContent = envContent.replace(
    /HTLC_STARK=.*/,
    `HTLC_STARK=${allContractsData["devnet"]?.["HTLC"]?.address || ""}`
  );
